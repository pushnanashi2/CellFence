...
CellFence is a manifest-driven architecture governance tool for TypeScript and JavaScript repositories edited in parallel by coding agents and humans. A manifest declares which cell owns which paths, which public entry other cells may import, and which resources each cell may touch. `cellfence check` turns those declarations into deterministic pass/fail results in CLI and CI, and an accepted baseline turns architectural growth into a review-gated event.
...
Version 0.x is deliberately narrow: Node.js ≥ 20; TypeScript and JavaScript only; one public entry per cell; repository-local cells; conservative static analysis (computed dynamic imports and non-literal resource paths surface as warnings, not resolutions). CellFence verifies the repository state agents leave behind; it does not prevent an agent from editing a path at runtime — combine it with worktree isolation and protected branches for a full control chain. Full list: [docs/limitations.md](docs/limitations.md).
