...
export function createClaim(options: ClaimCreateOptions, dependencies: ClaimOperationDependencies): ClaimCreateResult {
  ...
  try {
    ...
    if (store.claims.some((candidate) => candidate.id === claim.id)) {
      addFinding(findings, {
        ruleId: "CELLFENCE_CLAIM_INVALID",
        severity: "error",
        filePath: claimsPath,
        message: `claim id ${claim.id} already exists; choose a new --claim-id instead of replacing an existing lease`,
