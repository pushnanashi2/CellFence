...
This package is implemented against the observed CellFence 0.2.1 interfaces rather than a guessed manifest shape.

- `npx cellfence@0.2.1 init` generates `cellfence.manifest.json` with `schemaVersion: "cellfence.manifest.v1"`, optional `governance`, and `cells[]` containing `id`, `ownedPaths`, `publicEntry`, `publicSymbols`, `consumes`, and `producesArtifacts`.
