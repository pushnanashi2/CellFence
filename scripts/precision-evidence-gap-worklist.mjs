...
function usage() {
  console.error(`Usage:
  node scripts/precision-evidence-gap-worklist.mjs --preflight claim-preflight.json [--next-cycle summary.json] [--expansion-plan plan.json ...] [--out gaps.json] [--markdown gaps.md]
...
function externalManifestAttestationGaps(preflight) {
  ...
  for (const issue of preflight.issues || []) {
    const match = /^(.*) external manifest review requires review\.([a-zA-Z0-9_.-]+)$/.exec(issue);
    if (!match) continue;
    const subjectId = match[1];
    const field = `review.${match[2]}`;
...
function buildReport(options) {
  ...
  const manifestGaps = externalManifestAttestationGaps(preflight);
  ...
  if ((preflight.summary?.missingLabels || 0) > 0) {
    tasks.push(task(
      ...
      `${preflight.summary.missingLabels} selected findings need independent manual labels`,
      {
        ...
        worklist: nextCycle?.artifacts?.blindWorklist || preflight.worklist?.path || null,
