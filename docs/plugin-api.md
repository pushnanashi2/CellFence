...
External npm/local plugin auto-loading from manifest or CLI input is intentionally not enabled in v0.x; loading arbitrary code from repository-controlled configuration conflicts with the fail-closed dynamic-import rule. To avoid false confidence, manifest `plugins` and `extends` are rejected instead of silently ignored. Trusted programmatic callers can pass already-imported plugins through `checkRepository({ plugins })`. Plugin code is not sandboxed: adapters, rules, and reporters run in the same Node.js process and with the same filesystem/network privileges as the caller. Third-party wrappers must treat plugin selection as trusted-code configuration, not as repository-controlled input. When they use `checkChangedRepository`, they may provide a stable `pluginCacheKey`; without one, base-result caching is disabled so plugin code changes cannot reuse stale analysis.
