...
```bash
git tag -s v0.2.1 -m "CellFence v0.2.1"
...
```bash
gh workflow run npm-publish.yml --repo OWNER/REPOSITORY --ref v0.2.1 -f dry_run=true
```
...
```bash
gh workflow run npm-publish.yml --repo OWNER/REPOSITORY --ref v0.2.1 -f dry_run=false -f confirm_publish="publish 0.2.1"
```
...
1. Create a signed tag, for example `v0.2.1`.
