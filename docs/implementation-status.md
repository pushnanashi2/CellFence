...
| Strict governed-source ownership | enforced | `@cellfence/engine` with `manifest.governance.requireOwnership` | Enforces that included source files are owned by a cell and imports cannot target governed unowned source; `cellfence init` enables it, while disabled coverage emits a warning for compatibility | strict ownership fixtures |
...
| Rule severity policy | enforced | `@cellfence/schema` and `@cellfence/engine` | Repository, cell, path override, and CLI severity settings are supported; required rules cannot be weakened below error | plugin API tests |
...
| Agent claim leases | enforced | `@cellfence/engine` and `cellfence claim create/check/list` | Uses a JSON claim store, defaulting to `.cellfence/claims.json`; detects active cell/path/symbol/resource/artifact conflicts and can compare an agent's Git diff to its active claim | CLI tests |
