...
| Plugin API v1 | enforced | `@cellfence/plugin-api` and `@cellfence/engine` | Programmatic rules and adapters are supported through `checkRepository({ plugins })`; manifest-driven npm/local plugin auto-loading is reserved but not enabled in v0.x | plugin API tests |
