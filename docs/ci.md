...
The current repository runs its source-built CLI in `.github/workflows/ci.yml`, pins GitHub-hosted Actions to audited commit SHAs with tag comments, uses Node 20 for the main Linux jobs, runs Node 22 smoke tests on Linux, macOS, and Windows, and includes an additional Linux Node 24 smoke job for forward compatibility. The test job also runs the ReDoS regression check through `npm run security:redos` / `npm run redos:check`. The `ci-counts` job records uncovered coverage counts as an artifact; `npm run coverage:100` is intentionally not wired as a required CI job until the repository can pass that local gate. The reusable check Action runs the published npm CLI through its `version` input, not the source tree from the Action ref.
